...
- **内置固件快捷选择**：内置常见开发板固件，弹窗一键选用，免去本地找文件
...
2. **选择固件**：点击「选择固件」，选择编译好的 `.bin` 文件；也可点击「选择内置固件」，从内置列表一键选用
...
「选择内置固件」弹窗中的列表由 `src/core/builtinFirmwares.ts` 的 `BUILTIN_FIRMWARES` 数组驱动。新增内置固件只需：

1. 将固件 `.bin` 文件放入 `public/`
2. 在 `BUILTIN_FIRMWARES` 中追加一项（`id` / `name` / `url` / 可选 `description` / `defaultAddress`）

选择内置固件时会自动将烧录地址设为该项的 `defaultAddress`（完整镜像通常为 `0x0`）。
